                            ...
                            @foreach($exam_dist_type as $examdist)
                                <tr>
                                    ...
                                    <td>{{ $examdist->mark_value }}</td>
                                     <td>{{ $examdist->status==0? 'Unpublished':'Publishid'}}</td>
                                    <td>
                                        ...
                                        <form action="{{ route('exam_mark_distribution_type.destroy',$examdist->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You sure that to delete this Section'); ">
                                            @csrf
                                            @method('delete')
