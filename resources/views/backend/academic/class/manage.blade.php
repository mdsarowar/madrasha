        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('class.create') }}" class="btn btn-success float-end">
...
{{--                                        <a href="{{route('class.show',$class->id)}}" class="btn btn-{{$class->status==0? 'secondary':'primary'}} btn-sm py-0 px-1"><i class="uil-arrow-{{$class->status==0? 'down':'up'}}"></i></a>--}}
                                        <a href="{{ route('class.edit', $class->slug) }}" class="btn btn-primary btn-sm py-0 px-1"><i class="dripicons-document-edit"></i></a>
                                         <form action="{{ route('class.destroy', $class->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You sure that to delete this Class'); ">
                                            @csrf
                                            @method('delete')
