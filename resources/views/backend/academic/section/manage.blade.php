        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('section.create') }}" class="btn btn-success float-end">
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap w-100">
                            ...
                            @foreach($sections as $section)
                                <tr>
                                    ...
                                    <td>{!! \Illuminate\Support\Str::words($section->section_note,4,'....') !!} </td>
...
{{--                                        <a href="{{route('section.show',$section->id)}}" class="btn btn-{{$section->status==0? 'secondary':'primary'}} btn-sm py-0 px-1"><i class="uil-arrow-{{$section->status==0? 'down':'up'}}"></i></a>--}}
                                        <a href="{{ route('section.edit', $section->slug) }}" class="btn btn-info btn-sm py-0 px-1"><i class="dripicons-document-edit"></i></a>
                                        <form action="{{ route('section.destroy', $section->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You sure that to delete this Section'); ">
                                            @csrf
                                            @method('delete')
