                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap w-100">
                            ...
                            @foreach($ebooks as $ebook)
                                <tr>
                                    ...
                                    <td>{{ $ebook->academic_class_id }}</td>
                                    <td>{{ $ebook->name }}</td>
                                    <td>{{ $ebook->author_name }}</td>
                                    <td>{{ $ebook->book_code }}</td>
                                    <td>{{ $ebook->price }}</td>
                                    <td><img src="{{asset($ebook->cover_image)}}" style="height: 100px;width: 100px" alt=""></td>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('library_ebook.destroy', $ebook->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You sure that to delete this Section'); ">
                                            @csrf
                                            @method('delete')
