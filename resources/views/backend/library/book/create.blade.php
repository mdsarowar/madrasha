                                    ...
                                    <form action="{{isset($libraryBook) ? route('library_book.update', $libraryBook->id) : route('library_book.store') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group row mt-2">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <br/>
                                                <label for="" class="mt-2"> <input type="radio" name="status" value="1"  {{$errors->any() && old('status')==1? ('checked'): (isset($libraryBook) && $libraryBook->status == 1 ?'checked':'')}} >Published</label>
                                                <label for=""class="mt-2"> <input type="radio"  name="status" value="0"  {{$errors->any() && old('status')==0? ('checked'): (isset($libraryBook) && $libraryBook->status == 0 ?'checked':'')}} >UnPublished</label>
                                                <br>@error('status')<span class="text-danger f-s-12">{{$errors->first('status')}}</span> @enderror
                                            </div>
                                        </div>

                                        <div class=" form-group row mt-3 float-end ">
                                                <input type="submit" value="{{isset($academicStaff)?'Update Library Book':'Add Library Book'}}" class="btn btn-success">
                                        </div>
                                    </form>
                                </div>
                            </div>
