                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap w-100">
                            ...
                            @foreach($books as $book)
                                <tr>
                                    ...
                                    <td><img src="{{asset($book->cover_image)}}" style="height: 100px;width: 100px" alt=""></td>
                                    <td>{{ $book->self_no }}</td>
                                   <td>{{ $book->status==0? 'Unpublished':'Publishid'}}</td>
                                    <td>
                                        ...
                                        <form action="{{ route('library_book.destroy', $book->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You sure that to delete this Section'); ">
                                            @csrf
                                            @method('delete')
